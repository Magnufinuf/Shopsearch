...
const KNOWN_SIZES = ["xs", "s", "m", "l", "xl", "xxl", "2xl", "3xl", "xxxl"];
...
function search(query: string, items: Product[]): Product[] {
  ...
  const rawWords = q.replace(/under\s*\$?\d+/, "").split(/\s+/).filter(Boolean);

  // Skill ut størrelsesord fra vanlige søkeord
  const sizeWords = rawWords
    .filter((w) => KNOWN_SIZES.includes(w))
    .map(normalizeSize);
  const words = rawWords.filter((w) => !KNOWN_SIZES.includes(w));

  return items.filter((item) => {
    if (maxPrice !== null && item.price > maxPrice) return false;

    // Krev at produktet faktisk har den etterspurte størrelsen
    if (sizeWords.length > 0) {
      const itemSizes = item.sizes.map(normalizeSize);
      const hasSize = sizeWords.every((sw) => itemSizes.includes(sw));
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-zinc-50 px-6 py-16 dark:bg-black">
      <main className="flex w-full max-w-2xl flex-col items-center gap-8 text-center">
        ...
        >
          Har du en nettbutikk? Registrer den her
        </a>
